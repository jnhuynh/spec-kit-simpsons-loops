...
- Q: What observability signals must the phaser engine emit so operators can debug failed or surprising runs without reading source? → A: Structured logs to stderr (one JSON object per line) at INFO/WARN/ERROR levels: one INFO record per classified commit (commit hash, chosen task type, the rule name or operator tag that produced the classification, isolation kind, and any precedents consulted), one INFO record per emitted phase (phase number, branch name, base branch, ordered task list), one WARN record for each commit skipped due to an empty diff, and one ERROR record for each validation failure (offending commit hash, failing rule name, missing precedent or unsafe operation, and the canonical decomposition message when applicable). On any non-zero exit from the phaser stage the same ERROR payload MUST also be persisted as YAML to `<FEATURE_DIR>/phase-creation-status.yaml` (the same status file already defined for stacked-PR failures, with a `stage` field distinguishing `phaser-engine` from `stacked-pr-creation`) so the failure survives lost terminal output and is inspectable by the next pipeline run.
- Q: What authentication model and failure-mode handling must the stacked-PR creator use for the Git-hosting service? → A: The stacked-PR creator MUST delegate authentication entirely to the operator-configured Git-host CLI (for example `gh auth status` for GitHub) and MUST NOT read tokens from environment variables, files, or process arguments directly. Before creating any branch or pull request, the creator MUST probe authentication exactly once (a single `gh auth status` invocation, or the equivalent for other supported hosts) and MUST fail fast — before any branch or pull request is created — when (a) the CLI is not authenticated, or (b) the authenticated identity lacks `repo` scope on the target repository. Authentication failure, missing scope, rate-limit (HTTP 429 or abuse-detection signal), and generic network failure MUST each be recorded as a distinct `failure_class` value (`auth-missing`, `auth-insufficient-scope`, `rate-limit`, `network`) in `phase-creation-status.yaml` so operators can apply the correct remediation. Tokens, secrets, or any substring that could plausibly contain a credential MUST NEVER appear in any log line (FR-041), status file (FR-039, FR-042), or error message.
- Q: What hard upper bounds must the phaser engine enforce on feature size (number of classified commits and number of emitted phases) so the engine has a defined operating envelope and operators are pushed to split mega-features rather than producing unreviewable stacked-PR chains? → A: The phaser engine MUST accept feature branches with up to 200 non-empty (classified) commits and MUST emit up to 50 phases per feature; if either bound is exceeded, the engine MUST fail fast before emitting any manifest with a single `validation-failed` ERROR record whose `failing_rule` is `feature-too-large` and whose payload reports `commit_count` and `phase_count` so the operator can see which bound was exceeded. The error message MUST instruct the operator to split the feature into multiple smaller specs. These bounds apply to a single phaser run; they do not constrain the total commits across all features in the repository. Empty-diff commits skipped under FR-009 do not count toward the 200-commit bound.
...
- Stacked-branch or stacked-pull-request creation fails partway through (for example, transient network error or Git-host rate limit after some phases have already been created). The system halts immediately, leaves the already-created branches and pull requests intact, records the first uncreated phase and the underlying error in `<FEATURE_DIR>/phase-creation-status.yaml` (including the `failure_class` per FR-046), and exits with a non-zero status. The operator re-runs the stacked-creation step once the upstream condition is resolved; the re-run detects the existing branches and pull requests by name, skips them, and resumes from the first uncreated phase. The status file is deleted on full success.
- The operator runs the stacked-PR creator on a host where the Git-host CLI is not authenticated, or is authenticated as an identity lacking branch/pull-request creation scope. The creator fails fast before any branch is created (per FR-045), writes `phase-creation-status.yaml` with `failure_class: auth-missing` or `failure_class: auth-insufficient-scope` and `first_uncreated_phase: 1`, and exits with a non-zero status. No partially created branches or pull requests exist after this failure mode.
- A feature branch contains more than 200 non-empty commits, or its classification would emit more than 50 phases. The phaser engine fails fast before writing any manifest (per FR-048), records the violation as a `validation-failed` ERROR with `failing_rule: feature-too-large` (per FR-041), persists the same payload to `<FEATURE_DIR>/phase-creation-status.yaml` with `stage: phaser-engine` (per FR-042), and exits non-zero. The error message instructs the operator to split the feature into smaller specs; no partial manifest is left behind.
...
- **FR-039**: When stacked-branch or stacked-pull-request creation fails partway through (for example, network error, authentication failure, or rate limit from the Git host) the system MUST halt immediately on the failing phase, MUST NOT delete or modify any branches or pull requests already created in this run, MUST record the index of the first uncreated phase and the underlying error in a status file at `<FEATURE_DIR>/phase-creation-status.yaml` (with a top-level `stage` field set to `stacked-pr-creation` to distinguish this failure mode from a phaser-engine failure recorded under the same path per FR-042), and MUST exit with a non-zero status.
...
- **FR-044**: The stacked-PR creator MUST delegate Git-host authentication entirely to the operator-configured Git-host command-line interface (for example `gh` for GitHub) and MUST NOT read, accept, or transmit tokens through environment variables, files, command-line arguments, or any other channel under its own control.
- **FR-045**: Before creating any branch or pull request in a given run, the stacked-PR creator MUST probe authentication exactly once (a single `gh auth status` invocation, or the documented equivalent for other supported hosts) and MUST fail fast — before any branch is created or any pull request is opened — when (a) the host CLI reports no authenticated identity, or (b) the authenticated identity lacks the scope required to create branches and pull requests on the target repository (for GitHub, the `repo` scope). On a fail-fast exit under this requirement, no `phase-creation-status.yaml` partial-progress entry is written for any phase (because no phase was attempted), but a status file MUST still be written with `stage: stacked-pr-creation`, `failure_class` set per FR-046, and `first_uncreated_phase: 1` so the operator sees a consistent diagnostic surface.
- **FR-046**: When stacked-PR creation fails for any reason covered by FR-039 or FR-045, the status file at `<FEATURE_DIR>/phase-creation-status.yaml` MUST record a `failure_class` field whose value is one of: `auth-missing` (the host CLI is not authenticated), `auth-insufficient-scope` (the authenticated identity lacks the scope required to create branches or pull requests on the target repository), `rate-limit` (the host returned HTTP 429 or an abuse-detection signal), `network` (transport-level failure such as DNS, TCP reset, or TLS error), or `other` (any failure not matching the prior four). The classification MUST be derived from the host CLI's exit code or response, not guessed from the operator's environment.
- **FR-047**: No log line emitted by the phaser engine or the stacked-PR creator (per FR-041 and FR-043), no field written to `phase-creation-status.yaml` (per FR-039, FR-042, and FR-046), and no error message presented to the operator MAY contain a Git-host token, a session cookie, an authorization header, or any other credential material. The implementation MUST treat the entire authentication surface as opaque and never serialize values it received from the host CLI's authenticated channel beyond the structured fields enumerated in this specification.
...
- **FR-041**: The phaser engine MUST emit structured observability logs to stderr as one JSON object per line, with each record carrying at minimum a `level` field (`INFO`, `WARN`, or `ERROR`), a `timestamp` field (ISO-8601 UTC), an `event` field, and event-specific payload fields. The required event types are: `commit-classified` (INFO; payload: `commit_hash`, `task_type`, `source` which is either `operator-tag` or the inference `rule_name`, `isolation`, optional `precedents_consulted`), `phase-emitted` (INFO; payload: `phase_number`, `branch_name`, `base_branch`, ordered `tasks`), `commit-skipped-empty-diff` (WARN; payload: `commit_hash`, `reason`), and `validation-failed` (ERROR; payload: `commit_hash`, `failing_rule`, `missing_precedent` or `forbidden_operation`, and `decomposition_message` when the failure is a forbidden operation).
- **FR-042**: When the phaser stage exits with a non-zero status, the engine MUST also persist the final `validation-failed` ERROR payload as YAML to `<FEATURE_DIR>/phase-creation-status.yaml` with a top-level `stage` field set to `phaser-engine`, so that operators can diagnose the failure even when stderr was not captured. The same file is reused by stacked-branch/PR creation failures with `stage: stacked-pr-creation` (see FR-039), and on any subsequent fully successful re-run of the failing stage the file MUST be deleted.
- **FR-043**: The phaser engine MUST NOT write observability logs to stdout; stdout is reserved for structured machine-readable output that downstream pipeline stages may consume (for example, the manifest path on success). This separation MUST hold whether the phaser is invoked standalone (FR-008) or as part of the pipeline (FR-019).
- **FR-048**: The phaser engine MUST enforce a hard upper bound of 200 non-empty (classified) commits and 50 emitted phases per single phaser run; commits skipped under FR-009 (empty diffs) do not count toward the commit bound. When either bound would be exceeded, the engine MUST fail fast before writing any phase manifest and MUST emit a single `validation-failed` ERROR record (per FR-041) with `failing_rule: feature-too-large`, payload fields `commit_count` and `phase_count`, and a `decomposition_message` that instructs the operator to split the feature into multiple smaller specs. The same payload MUST be persisted to `<FEATURE_DIR>/phase-creation-status.yaml` per FR-042 with `stage: phaser-engine`. These bounds constrain a single phaser run only; they place no limit on the total number of features in the repository.
...
- **SC-011**: For every phaser run that produces a manifest, the engine emits at minimum one `commit-classified` log record per non-empty commit and one `phase-emitted` log record per phase, all parseable as JSON by a regression test that pipes stderr through a JSON-line parser; for every phaser run that fails on validation, the engine emits exactly one `validation-failed` ERROR record on stderr and persists the same payload to `<FEATURE_DIR>/phase-creation-status.yaml`, verified by a regression test that asserts both the stderr line and the file contents agree on the offending commit hash and the failing rule.
- **SC-012**: When the stacked-PR creator runs against a Git host where the operator-configured CLI is not authenticated, the creator exits non-zero before any branch or pull request is created and writes `<FEATURE_DIR>/phase-creation-status.yaml` containing exactly `stage: stacked-pr-creation`, `failure_class: auth-missing`, and `first_uncreated_phase: 1`; verified by a regression test that simulates an unauthenticated host CLI and asserts no branches were created and no pull requests were opened.
- **SC-013**: A regression test that scans every log line emitted by the phaser engine and the stacked-PR creator across the full fixture set, plus every byte of every `phase-creation-status.yaml` produced by failure-mode fixtures, finds zero substrings matching common credential patterns (Git-host personal access token prefixes, bearer-token headers, base64-encoded session cookies); verifying FR-047.
- **SC-014**: A regression test that constructs a synthetic feature branch with 201 non-empty commits, and a second test that constructs a flavor and commit set whose classification would emit 51 phases, both verify that the phaser engine exits non-zero before writing any phase manifest, emits exactly one `validation-failed` ERROR record with `failing_rule: feature-too-large` on stderr, and persists the same payload to `<FEATURE_DIR>/phase-creation-status.yaml` with `stage: phaser-engine`; verifying FR-048.
