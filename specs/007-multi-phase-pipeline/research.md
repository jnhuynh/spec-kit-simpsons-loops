...
- **File exists**: Insert the phaser step **after** the existing simplify/security-review polish phases and **before** marge — so the pipeline order becomes `ralph → simplify → security-review → phaser → marge`. Placing phaser after the polish phases (rather than between ralph and simplify) is required so that any commits created by simplify or security-review are classified into the manifest; otherwise the manifest's commit set would not match the branch's commit set, FR-002 determinism would be defeated, and unreviewed polish commits would slip into the stacked PRs outside any phase. After phaser succeeds, invoke marge `N+1` times: once per phase with `--phase N` (FR-022), then once holistically without the flag (FR-023). The holistic pass MUST run after all per-phase passes complete.
