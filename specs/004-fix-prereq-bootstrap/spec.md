...
- What happens when a user invokes the root-level `pipeline.sh` (instead of `.specify/scripts/bash/pipeline.sh`) with `--from specify`? The root-level copy has a pre-existing `REPO_ROOT` derivation issue (`SCRIPT_DIR/../../..` resolves incorrectly when run from the repo root). This bootstrap fix is applied identically to both copies, but the root-level copy's incorrect `REPO_ROOT` may cause path resolution failures unrelated to this fix. This is a known pre-existing limitation — a separate chore should convert the root-level copy to a thin wrapper that delegates to `.specify/scripts/bash/pipeline.sh`.
