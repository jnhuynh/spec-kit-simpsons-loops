...
**Prefix Pattern Note**: `pipeline.sh:resolve_feature_dir()` (lines 302-359) uses a 4-char alphanumeric regex `^([a-z0-9]{4})-` (line 333) while `common.sh:find_feature_dir_by_prefix()` (used by `check-prerequisites.sh`) uses a 3-digit numeric regex `^([0-9]{3})-`. **Important**: The current naming convention uses 3-digit numeric prefixes (e.g., `004-fix-prereq-bootstrap`), which means `pipeline.sh`'s 4-char regex never matches any existing branch or directory. In practice, `resolve_feature_dir()` always falls through the glob loop to the exact-match check (`specs/$branch`) at line 344. This is a pre-existing issue unrelated to this fix. Since `check-prerequisites.sh` is out of scope, only `pipeline.sh` is modified. When constructing a prospective path during bootstrap (no existing directory), the implementation uses the full branch name (`specs/$branch`) in the bootstrap fallback (lines 349-355) rather than prefix glob matching, because (a) the directory does not exist yet for glob expansion to find, and (b) the prefix glob loop would not match the 3-digit naming convention anyway.
**Branch Validation Note**: `check-prerequisites.sh` calls `check_feature_branch()` (in `common.sh`) which validates the branch matches `^[0-9]{3}-` BEFORE reaching the `--paths-only` early-exit code path (line 84 vs lines 87-101). This means `--paths-only` only works on feature branches. On non-feature branches (e.g., `main`), `speckit.pipeline.md` MUST skip calling `check-prerequisites.sh` entirely and proceed with an unresolved `FEATURE_DIR`, allowing the specify step to create the branch and directory.
