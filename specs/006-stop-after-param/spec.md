...
- **FR-008**: The pipeline's completion report MUST list all six pipeline steps with a per-step status: `executed`, `skipped` (artifact already exists), or `stopped-by-param` (not executed due to `--stop-after`). When execution was stopped early, the report MUST clearly indicate which step was the last executed and that remaining steps were stopped by the `--stop-after` parameter.
