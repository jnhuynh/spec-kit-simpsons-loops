...
- [ ] T008 [US1] Verify default behavior preservation in `.claude/commands/speckit.pipeline.md` — when `--stop-after` is not provided, ensure `STOP_AFTER_STEP` is empty/unset so all stop checks are no-ops and the pipeline behaves identically to the current implementation (FR-007)
