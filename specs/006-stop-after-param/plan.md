...
| Test-First Development | **N/A** | This feature modifies a Markdown command file (`.claude/commands/speckit.pipeline.md`) interpreted by Claude CLI, not executable code. The constitution's test-first requirement targets executable logic — "new service functions, business logic, hooks, utilities, and bug fixes." Markdown command files are declarative instructions, not testable code units. **Alternative verification** substitutes for automated unit tests: (1) the quickstart.md verification checklist (`specs/006-stop-after-param/quickstart.md`) defines 17 concrete acceptance checks covering argument parsing, validation, execution, reporting, and default behavior; (2) manual integration test invocations are documented in Phase 3-5 independent test descriptions in tasks.md; (3) T015 explicitly runs the full verification checklist against the implementation. This approach satisfies the constitution's intent (proving implementations work before declaring them complete) through the appropriate means for this medium. |
