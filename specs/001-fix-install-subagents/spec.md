...
4. **Given** any loop command is running, **When** the maximum iteration limit is reached (10 for homer/lisa, `incomplete_tasks + 10` for ralph) without the promise tag being returned and without stuck detection triggering, **Then** the loop aborts with a clear message reporting the iteration count and suggesting manual review
...
- What happens if a loop reaches its maximum iteration limit without the sub agent returning the promise tag and without stuck detection triggering? The loop aborts with a clear message reporting the iteration count and suggesting manual review. Homer and lisa use a fixed limit of 10 iterations; ralph uses a dynamic limit of `incomplete_tasks + 10` (per FR-012). This prevents runaway execution when outputs vary enough to bypass stuck detection but never converge on the completion signal.
...
- Q: What is the maximum iteration limit for loop commands to prevent runaway execution when stuck detection does not trigger? → A: All loop commands MUST enforce a maximum iteration limit per loop invocation. Homer and lisa use a fixed limit of 10 iterations. Ralph uses a dynamic limit of `incomplete_tasks + 10` to allow completion of all tasks with a safety buffer (capping at exactly 10 would prevent completing features with more than 10 tasks). When the limit is reached, the loop aborts with a clear message reporting the iteration count and suggesting manual review. This safeguards against non-converging outputs that vary enough to bypass stuck detection but never reach the promise tag.
...
- **FR-012**: All loop commands MUST enforce a maximum iteration limit per loop invocation — homer and lisa use a fixed limit of 10 iterations; ralph uses a dynamic limit of `incomplete_tasks + 10` (to allow completion of all tasks with a buffer). When the limit is reached, the loop aborts with a clear message reporting the iteration count and suggesting manual review
...
- **SC-007**: All 4 loop command files enforce a maximum iteration limit (homer/lisa: 10, ralph: `incomplete_tasks + 10`), aborting with a clear message when the limit is reached
