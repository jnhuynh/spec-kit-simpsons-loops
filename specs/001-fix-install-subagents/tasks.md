...
- [x] T036 [US4] Verify all file paths referenced in `README.md` correspond to actual files in the repository per FR-007 and SC-005 — VERIFIED COMPLETE: all 15 source files referenced in README.md exist at repository root and agents/ (4 bash scripts, 5 agent definitions, 4 loop commands, setup.sh, gitignore); all destination paths reference standard install locations (.specify/scripts/bash/, .claude/agents/, .claude/commands/) created by setup.sh; all 4 slash commands (/speckit.homer.clarify, /speckit.lisa.analyze, /speckit.ralph.implement, /speckit.pipeline) correspond to their respective .md files; .specify/logs/ logging path is consistent across all 4 bash scripts; .gitignore entries in README match the gitignore template file
- [x] T037 [US4] Verify `README.md` describes the recommended workflow (slash commands with sub agents) and the bash script fallback accurately per FR-007 — VERIFIED & FIXED: README accurately describes both the recommended workflow (slash commands section lines 129-175: each loop has a corresponding slash command that orchestrates iterations using the Agent tool with fresh context windows, correct prerequisites for each command, pipeline auto-detection and --from resume) and the bash script fallback (lines 186-217: standalone alternative spawning `claude --agent` CLI processes, correct usage examples for all 4 scripts). Added missing `--quality-gates <cmd>` flag to the pipeline options table to match pipeline.sh's actual flags. All descriptions match actual implementation behavior.
- [x] T038 [US4] Verify terminology in `README.md` is consistent with loop command files and agent definitions -- use the Terminology Map from data-model.md Entity 4 as the reference per FR-008 and FR-009 — VERIFIED & FIXED: replaced 3 instances of deprecated terminology in README.md: (1) line 47 "loop scripts" changed to "bash loop scripts" (canonical term), (2) line 59 comment "Shell scripts" changed to "Bash loop scripts" (canonical term), (3) line 72 comment "Claude Code commands" changed to "Loop commands" (canonical term for the speckit.*.md files), (4) line 110 heading "loop scripts" changed to "bash loop scripts". All 6 canonical terms from Terminology Map (Agent tool, sub agent, loop command, bash loop script, promise tag, stuck detection) now used correctly throughout README.md. Zero occurrences of deprecated terms (Task tool, child agent, completion signal) remain.
- [x] T039 [US4] Verify `README.md` describes the 13 distribution files and their correct destinations per the Distribution File Manifest in spec.md — VERIFIED COMPLETE: README Option B (Manual) section (lines 59-76) enumerates all 13 distribution files with correct source and destination paths matching the Distribution File Manifest in spec.md: 4 bash loop scripts to .specify/scripts/bash/ (lines 60-63), 5 agent definitions to .claude/agents/ (lines 66-70), 4 loop commands to .claude/commands/ (lines 73-76). Option A (Automated) section (line 47) accurately describes setup.sh copying "all files (bash loop scripts, pipeline, agent definitions, and loop commands)" by category. chmod section (lines 82-85) lists all 4 bash scripts. No discrepancies found.
...
- [x] T040 Run a global search across all `.md` files at repository root and in `agents/` for the deprecated term "Task tool" and confirm zero occurrences remain per FR-008 and FR-009 — VERIFIED COMPLETE: searched all .md files at repository root (speckit.homer.clarify.md, speckit.lisa.analyze.md, speckit.ralph.implement.md, speckit.pipeline.md, README.md) and in agents/ (homer.md, lisa.md, ralph.md, plan.md, tasks.md) for "Task tool"; zero occurrences found in any distribution file. Only matches are in specs/001-fix-install-subagents/ documentation files where "Task tool" is referenced as a deprecated term — these are correct and expected.
- [x] T041 Run a global search across all `.md` files for any other deprecated synonyms from the Terminology Map (data-model.md Entity 4): "child agent" used as a synonym for "sub agent", "loop script" used to describe loop commands (not bash scripts) — VERIFIED COMPLETE: searched all distribution .md files (4 loop commands at root, 5 agent definitions in agents/, README.md) for deprecated terms: (1) "child agent" — zero occurrences in any distribution file, only found in specs/ documentation where it is correctly listed as a deprecated synonym; (2) "loop script" without "bash" prefix — zero occurrences in any distribution file, all uses in README.md correctly use the canonical "bash loop scripts"; (3) "Task tool" — zero occurrences in any distribution file, only found in specs/ documentation referencing the deprecated term. All distribution files use canonical terminology consistently.
- [x] T042 Verify all 4 bash loop scripts (`homer-loop.sh`, `lisa-loop.sh`, `ralph-loop.sh`, `pipeline.sh`) are unchanged except for any bug fixes -- bash scripts should NOT have "Task tool" -> "Agent tool" replacements since they are shell scripts, not Markdown — VERIFIED COMPLETE: git diff main..HEAD shows zero changes to all 4 bash scripts on this branch; grep confirms no "Task tool" or "Agent tool" strings exist in any bash script (these are shell scripts, not Markdown); all scripts pass bash -n syntax check
- [x] T043 Run the quickstart verification checklist from `specs/001-fix-install-subagents/quickstart.md` to confirm all 7 verification items pass — VERIFIED COMPLETE: all 7 items pass: (1) `grep -r "Task tool" *.md agents/*.md` returns zero results, (2) no loop command file mentions "Abort after 3 consecutive failures", (3) all 4 loop command files reference "Agent tool" with `subagent_type: general-purpose`, (4) README terminology matches loop command files (2 "Agent tool", 3 "sub agent", 0 "Task tool"), (5) setup.sh runs successfully against test dir with .claude/ and .specify/ scaffolding (exit 0, all 13 files copied, 4 scripts executable, .gitignore updated, settings.local.json has 4 permissions), (6) setup.sh is idempotent (second run exits 0, skips existing gitignore and settings entries), (7) all file paths in README correspond to actual files in the repository
- [x] T044 Validate all success criteria from spec.md are met: SC-001 through SC-007 — VERIFIED COMPLETE: (SC-001) setup.sh runs against test directory with .claude/ and .specify/ scaffolding, exits 0, all 13 files copied to correct locations, 4 scripts executable, .gitignore has marker, settings.local.json has 4 permissions; (SC-002) second run is idempotent — .gitignore and settings.local.json identical, no errors, no duplicates; (SC-003) all 4 loop command files specify `subagent_type: general-purpose` (homer line 43, lisa line 51, ralph line 51, pipeline lines 67/76/83/90/107); (SC-004) all 4 files have AUTONOMOUS EXECUTION and STRICT SEQUENTIAL EXECUTION blocks; (SC-005) all 16 source files referenced in README exist in repository, all destination paths are correct; (SC-006) zero occurrences of deprecated "Task tool" or "child agent" in any distribution file (README, loop commands, agent definitions); (SC-007) iteration limits enforced: homer=10, lisa=10, ralph=incomplete_tasks+10, pipeline per-step limits match
