...
- [x] T019 [US3] Verify in `claude-agents/ralph.md` (per T009) that single-phase features (tasks without `[phase-N]` tags) produce commits without `Phase:` trailers per FR-007; do not silently default to `Phase: 1` in the commit text
...
- [x] T021 [US3] Run V-001 (single-phase regression) from `specs/008-feat-multi-phase-deploys/quickstart.md` against `001-consistency-cleanup` (or another existing single-phase feature) to confirm `plan.md` does not acquire a `## Deploy Phases` section, `tasks.md` carries no `[phase-N]` tags, Ralph commits have no `Phase:` trailers, Marge produces `review-report.md` with Phase column `-` for every row, the split step opens exactly one PR against `main` and creates no extra branches, and `split-report.md` contains exactly one row with `Phase` = `single` per SC-003 and FR-022
