...
- **SC-006**: The split step refuses to open or update a pull request for any phase carrying an unresolved high-severity migration-safety finding, and the refusal message identifies which finding blocked which phase.
...
- Migration-safety findings rated below high severity (medium, low, informational) do NOT block the split step from opening pull requests; they are surfaced as review comments for the human reviewer to address. Only high-severity findings gate the split step.
