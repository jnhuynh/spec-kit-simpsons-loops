...
- **FR-017**: The split step MUST be idempotent: re-running it MUST update existing phase branches and pull requests in place rather than creating duplicates. The split step identifies existing phase branches by the deterministic naming convention `NNNN-<type>-<slug>-phaseK` defined in FR-015; a remote branch matching that name is treated as an existing phase branch to update, and no remote branch with that name means a new phase branch to create. To update a phase branch in place when its commit set has changed (commits added, removed, or amended), the split step MUST deterministically rebuild the phase branch by resetting it to the appropriate base (`main` for phase 1, the previous phase branch for phase K>1) and re-cherry-picking that phase's current commits from the feature branch, then force-push the rebuilt phase branch to the remote with `--force-with-lease`. Phase branches are pipeline-managed artifacts: humans MUST NOT commit directly to `NNNN-<type>-<slug>-phaseK` branches, and the split step's force-push will overwrite any such edits. If a phase branch's pull request is already merged into `main`, the split step MUST skip rebuilding that phase (treat it as immutable history) and report the skip in its output. When the split step encounters an error while creating or updating a phase branch or its pull request (for example, a `gh pr create` failure on phase K), it MUST fail fast on the first error, leave already-completed phase branches and pull requests in place, and report which phase failed and why so that an idempotent re-run can resume from the failed phase.
- **FR-018**: The split step MUST refuse to open or update a pull request for any phase that has an unresolved high-severity review finding tagged to it. The split step MUST read findings from the persisted review report at `<FEATURE_DIR>/review-report.md` (see FR-012a). If the review report is missing, the split step MUST refuse to run and instruct the author to run the review step first. The split step MUST report which finding blocked which phase. For single-phase features (where findings carry no phase tag per FR-012a), the split step MUST treat every unresolved high-severity finding as gating the single pull request and MUST refuse to open or update that pull request until the finding is resolved; this preserves uniform safety gating across single-phase and multi-phase modes.
...
- **FR-019a**: The split step MUST persist a structured outcome report to `<FEATURE_DIR>/split-report.md` on every run, overwriting any prior copy, and MUST mirror a concise human-readable summary to stdout. The machine-readable surface of the report MUST be a single GitHub Flavored Markdown table with the exact column headers `| Phase | Status | Branch | PR URL | Reason |` in that order, one row per phase enumerated by the run, with pipe (`|`) characters in cell values escaped as `\|`. The file MAY contain additional human-readable prose before or after the table. Consumers MUST locate the table by its exact header row and parse rows until the next blank line or end of file. For each phase enumerated by the run, the row MUST include:
  ...
  - **Status**: one of `created` (new phase branch and pull request opened in this run), `updated` (existing phase branch rebuilt and pull request updated in place), `unchanged` (existing phase branch and pull request already matched the recomputed state and no force-push was required), `skipped-merged` (the phase branch's pull request was already merged into `main` per FR-017), `gated` (the phase was blocked by an unresolved high-severity finding per FR-018), or `failed` (the split step failed fast on this phase per FR-017's fail-fast rule).
