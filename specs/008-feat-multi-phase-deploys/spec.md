...
- **FR-019a**: The split step MUST persist a structured outcome report to `<FEATURE_DIR>/split-report.md` on every run, overwriting any prior copy, and MUST mirror a concise human-readable summary to stdout. For each phase enumerated by the run, the persisted report MUST include:
  - **phase number**: the integer phase number when multi-phase; the literal value `single` when the feature is single-phase.
  ...
  - **branch**: the phase branch name (`NNNN-<type>-<slug>-phaseK` when multi-phase; the feature branch name when single-phase). Omitted only when the run failed before resolving the branch name.
  - **pull request URL**: the URL of the corresponding pull request when one exists. Omitted for `gated` or `failed` entries that did not result in a pull request, and for `unchanged` entries when the existing pull-request URL was not re-fetched.
  - **reason**: a single-sentence human-readable explanation. Required for every non-success status (`skipped-merged`, `gated`, `failed`); optional for `created`, `updated`, and `unchanged`.
  
  The persisted report MUST be machine-readable enough that the pipeline orchestrator can extract `(phase number, status, reason)` for every entry without re-running the split step. The pipeline orchestrator MUST treat absence of `<FEATURE_DIR>/split-report.md` after a split-step invocation as a split-step failure.
...
- **Split Report**: A persisted artifact at `<FEATURE_DIR>/split-report.md` produced by the split step on every run. Lists every phase enumerated by the run with its terminal status (`created`, `updated`, `unchanged`, `skipped-merged`, `gated`, `failed`), branch name, pull-request URL when applicable, and a reason for any non-success status. Read by the pipeline orchestrator to determine whether the split step succeeded without re-running it.
