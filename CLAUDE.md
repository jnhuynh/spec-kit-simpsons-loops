...
- Bash 4+ (shell scripts), Markdown (command/agent files) + Claude CLI (`claude` command) + Claude CLI (`claude` command), Agent tool, Bash tool, `git` (with trailer support — Git 2.32+ for `--format='%(trailers:key=Phase,valueonly)'`), `gh` CLI (authenticated for the repository), standard Unix utilities (`grep`, `sed`, `awk`, `test`) (008-feat-multi-phase-deploys)
