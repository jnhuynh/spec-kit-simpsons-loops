...
A config-backed pack names its data file (e.g. `Config: .specify/marge/config/linked-files.yml`) and reads it as rule data. Keeping the data here — separate from the rule prose in `../checks/` — lets one pack serve many groups/cases without editing the rule itself.
